...
private val cache = ConcurrentHashMap<CacheKey, Result>()
...
abstract class BaseCachingValidator : Plugin {
    ...
    private fun checkCached(params: ECParameterSpec, supplementary: Supplementary? = null): Result {
        return cache.computeIfAbsent(CacheKey(params)) { doCheck(params, supplementary) }
    }
...
class FieldValidator : BaseCachingValidator() {
    ...
    override fun doCheck(params: ECParameterSpec, supplementary: Supplementary?): Result {
        ...
        return when {
            field is ECFieldF2m -> Vulnerable("SafeCurves required elliptic curves to be over prime fields")
            ...
            !(field as ECFieldFp).p.isProbablePrime(7) -> Vulnerable("SafeCurves requires 'p' parameter of elliptic curve to be prime, used value")
            else -> Validated()
        }
    }
}
...
class ABValidator : BaseCachingValidator() {
    ...
    override fun doCheck(params: ECParameterSpec, supplementary: Supplementary?): Result {
        ...
        if (((curve.a.pow(3) * `4`) + (curve.b.pow(2) * `27`)) eq `0`)
            return Vulnerable("4*a^3+27*b^2 is 0, this params does not define valid elliptic curve")

        if (curve.field is ECFieldFp && !isPointOnCurve(curve, params.generator))
            return Vulnerable("Base point (x=${params.generator.affineX.toString(16)}, y=${params.generator.affineY.toString(16)}) is not on specified curve")
...
class OrderValidator : BaseCachingValidator() {
    ...
    override fun doCheck(params: ECParameterSpec, supplementary: Supplementary?): Result {
        if (!params.order.isProbablePrime(7))
            return Vulnerable("Curve is vulnerable to attacks using Pohlig–Hellman algorithm if it's order is not prime")

        if (params.order < `2^200`)
            return Vulnerable("Curves with order less than 2^200 are vulnerable to attacks using Pollard's rho algorithm")
...
class SeedValidator : BaseCachingValidator() {
    ...
    override fun doCheck(params: ECParameterSpec, supplementary: Supplementary?): Result {
        ...
        if (curve.seed?.takeIf { it.isNotEmpty() } == null)
            return Vulnerable("No seed given, EC params are not verifiable random")
        ...
        if ((curve.b.pow(2) * c - curve.a.pow(3)) % field.p != `0`)
            return Vulnerable("Params a=${curve.a.toString(16)} and b=${curve.b.toString(16)} are not valid against given seed=${iSeed.toString(16)}")
...
class TransferSafetyValidator : BaseCachingValidator() {
    ...
    override fun doCheck(params: ECParameterSpec, supplementary: Supplementary?): Result {
        ...
        if ((params.order * bi(params.cofactor)) eq field.p)
            return Vulnerable("Curve is vulnerable to attacks based on additive transfer (Smart-attack)")

        val gcd = params.order.gcd(field.p)
        if (gcd ne `1`)
            return Vulnerable("Curve is vulnerable to MOV-attacks. GCD of order and 'p' must be 1")
        ...
        if (((params.order - `1`) / ed) > `100`)
            return Vulnerable("Curve is vulnerable to MOV-attacks, embedding degree is too low ($ed)")
...
class DiscriminantValidator : BaseCachingValidator() {
    ...
    override fun doCheck(params: ECParameterSpec, supplementary: Supplementary?): Result {
        ...
        if (D >= `0`)
            return Vulnerable("Discriminant is not negative")

        if (D >= `2`.pow(100).negate())
            return Vulnerable("CM-field discriminant is too small ($D less than 2^100)")
...
class TwistValidator : BaseCachingValidator() {
    ...
    override fun doCheck(params: ECParameterSpec, supplementary: Supplementary?): Result {
        ...
        if (p.gcd(twistOrder) ne `1`)
            return Vulnerable("Curve is not twist-safe. GCD of twist order and 'p' must be 1")
        ...
        if (ln(twistOrder.toBigDecimal().multiply(pi4), s3) / ln(bd4, s3) < bd100)
            return Vulnerable("Curve twist is not safe to rho attacks")
        ...
        if (ln(precompBD + sqrt(pi4 * jointBD, s3), s3) / ln2(s3) < bd100)
            return Vulnerable("Curve twist is not safe to joint rho attacks")
...
fun isPointOnCurve(curve: EllipticCurve, point: ECPoint): Boolean {
    ...
    return ((point.affineY.pow(2) - (point.affineX.pow(3) + (curve.a * point.affineX) + `1`)) % p) eq `0`
}
