...
public class SafeEcClient implements AutoCloseable {
    ...
    public static SafeEcClient getInstance() {
        if (INSTANCE == null)
            INSTANCE = new SafeEcClient(System.getProperty("server.host"), Integer.getInteger("server.port", 7999));
    ...
    public SafeEcClient(String host, Integer port) {
        this.host = host;
        this.port = port;
    ...
    private Response receive() {
        return read(() -> {
            ...
            return mapper.readValue(decoded, Response.class);
        });
    }
